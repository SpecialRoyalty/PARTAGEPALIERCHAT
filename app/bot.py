...
from app.models import (
    Base, Group, GroupStatus, Campaign, CampaignGroup, InviteLink, Invite,
    InviteStatus, BotUser
)
    ...
    async with SessionLocal() as session:
        ...
        if payload and payload.startswith("get_"):
            ...
            await update.message.reply_text(
                ...
                "Conseil : partage ton lien dans tes groupes, stories, canaux et DM."
            )
            return
...
async def publish_campaign(context: ContextTypes.DEFAULT_TYPE, group_id: int):
    async with SessionLocal() as session:
        ...
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton(campaign.button_text, url=start_url)]
        ])
        ...
        context.job_queue.run_once(
            ...
            name=f"delete_ad_{group.telegram_chat_id}_{msg.message_id}",
        )
...
async def my_chat_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with SessionLocal() as session:
        ...
        if new_status in ("member", "administrator"):
            group.status = group.status or GroupStatus.pending
            await session.commit()
            await notify_admins(
                context.bot,
                f"🆕 Bot ajouté dans un groupe\n\n"
                f"Groupe: {chat.title}\nChat ID: {chat.id}\n\n"
                f"Va dans l’admin panel pour valider."
            )
        else:
            await session.commit()
...
async def handle_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with SessionLocal() as session:
        ...
        if banned_word:
            ...
            await notify_admins(
                context.bot,
                f"🚫 Ban auto\nUser: {joined_tg_user.id} @{joined_tg_user.username}\n"
                f"Groupe: {chat.title}\nMot détecté: {banned_word}"
            )
            await session.commit()
            return
    ...
    context.job_queue.run_once(
        ...
        name=f"validate_invite_{invite_id}",
    )
        ...
        try:
            ...
            if member.status in ("member", "administrator", "restricted"):
                ...
                await context.bot.send_message(
                    ...
                    f"✅ Nouvelle personne validée dans {group.title}.\n\n"
                    f"Total validé : {count + 1}\n"
                    "Continue, tu te rapproches du prochain palier."
                )
